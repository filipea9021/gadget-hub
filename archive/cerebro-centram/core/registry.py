...
class SkillRegistry:
    ...
    def _register_default_skills(self) -> None:
        ...
        # ── Skill Data Core (Supabase — memória, media, dados) ──
        self.register(SkillInfo(
            name="data_core",
            description="Sistema central de dados com Supabase. Gere memória persistente (logs, notas, eventos), media (imagens, vídeos, documentos) e dados genéricos. Funciona como a 'base de dados inteligente' de todo o sistema.",
            runtime=SkillRuntime.PYTHON,
            capabilities=[
                "guardar memória", "pesquisar memórias", "registar evento",
                "criar log", "histórico de operações", "notas do sistema",
                "upload de imagem", "upload de vídeo", "gerir media",
                "guardar dados", "consultar dados", "base de dados",
                "Supabase", "storage", "persistência",
            ],
            keywords=[
                "memória", "memoria", "lembrar", "guardar", "registar", "log",
                "dados", "data", "base de dados", "supabase", "storage",
                "upload", "media", "galeria", "ficheiro", "histórico",
            ],
            module_path="skills.data_core.agent",
            metadata={"type": "supabase_backend", "modules": ["memory", "media", "data"]},
        ))
