...
class CerebroCentral:
    ...
    async def _execute_python_skill(
        ...
        elif skill_info.name == "data_core":
            from skills.data_core.agent import DataCoreSkill

            skill = DataCoreSkill(self.settings)
            result = await skill.execute(command, context)

            # Normalizar resposta para o formato do Cérebro Central
            status = "sucesso" if result.get("status") in ("ok", "success") or result.get("code") in (200, 201) else "erro"
            return {
                "status": status,
                "skill": "data_core",
                "dados": result.get("data", result),
                "mensagem": result.get("message", str(result.get("code", ""))),
            }
